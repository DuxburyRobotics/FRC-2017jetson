...
void GripPipeline::process(cv::Mat source0){
	...
	double hsvThresholdHue[] = {71.2230215827338, 100.13651877133103};
...
void GripPipeline::setsource0(cv::Mat &source0){
	VideoCapture cap; //
    	...
    	cap.open("http://10.49.8.10/video?x.mjpeg");
    	if (!cap.isOpened())  // if not success, exit program
    	{
       	 	cout << "Cannot open the video cam" << endl;
        	return -1;
    	}else{
		cout << "Open the video is good" << endl;
	}	
	cap.copyTo(this->source0);
}
/**
